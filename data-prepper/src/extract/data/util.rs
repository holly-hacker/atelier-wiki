    ...
    F: FnOnce(roxmltree::Document) -> anyhow::Result<T>,
{
    ...
    file.read_to_string(&mut xml_string)
        .context("read xml file to string")?;

    let document = roxmltree::Document::parse(&xml_string).context("parse xml file")?;
    let result = parse_fn(document)?;

    Ok(result)
}

// TODO: instead of having a separate function, check the first header of the xml file
// a SHIFT_JIS file should start with `<?xml version="1.0" encoding="SHIFT-JIS"?>`
pub fn read_xml_shift_jis<T, F>(
    ...
    F: FnOnce(roxmltree::Document) -> anyhow::Result<T>,
{
    ...
    file.read_to_end(&mut xml_buffer)
        .context("read xml file to buffer")?;
    let xml_string = encoding_rs::SHIFT_JIS.decode(&xml_buffer).0;
